...
namespace configuration
{
	...
	constexpr auto sampleBit              = AUDIO_CONFIG_PREFIX ".sampleBit";
...
/*****************audio struct**************************/
    ...
    using RTPSessionDatas = std::queue<RTPSessionData>;
...
#pragma pack(2)
    ...
    {
        char            riff[4];                // = "RIFF"
        int             chunk_size;             // = File total size - 8(chunk id and chunk size)
        char            wave[4];                // = "WAVE"
        // fmt sub-chunk
        char            fmt[4];                 // = "fmt"
